...
- ❌ Two diagram instances whose placement rects overlap (e.g. two `AgentCoordination` each with `{x:200, w:1200}` — their rects both span x=200..1400 and overlap). Every composite takes `{x, y, w, h}`; each rect must be non-overlapping and meet the composite's minimum per `sizingNotes`.
