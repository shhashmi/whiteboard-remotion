...
4. **Diagrams take a placement rect.** Every composite accepts `{x, y, w, h}` describing the area it may fill; it auto-fits its internal nodes and labels to the rect. Check `sizingNotes` in the asset registry for each composite's **minimum rect size** per pattern × content count. If you provide a rect smaller than the minimum, the validator rejects the plan — resize the rect, choose a simpler pattern, or shorten labels.
